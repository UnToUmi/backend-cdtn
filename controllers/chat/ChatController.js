  ...
  add_customer_friend = async (req, res) => {
    ...
    try {
      if (sellerId !== "") {
        ...
        let userRecord = await sellerCustomerModel.findOne({ myId: userId });
        if (!userRecord) {
          userRecord = new sellerCustomerModel({
            myId: userId,
            myFriends: [],
          });
          await userRecord.save();
        }
        let sellerRecord = await sellerCustomerModel.findOne({
          myId: sellerId,
        });
        if (!sellerRecord) {
          sellerRecord = new sellerCustomerModel({
            myId: sellerId,
            myFriends: [],
          });
