...
class dashboardController {
  get_admin_dashboard_data = async (req, res) => {
    ...
    try {
      ...
      const sellersByMonth = Array(12).fill(0);
      sellers.forEach((seller) => {
        const month = new Date(seller.createdAt).getMonth(); // getMonth() returns 0-11
        sellersByMonth[month]++;
      });

      const orders = await customerOrder.find({});
      const ordersByMouth = Array(12).fill(0);
      orders.forEach((seller) => {
        const month = new Date(seller.createdAt).getMonth(); // getMonth() returns 0-11
        ordersByMouth[month]++;
      });

      const transactions = await myShopWallet.find({});
      const transactionAmountsByMonth = Array(12).fill(0);
      transactions.forEach((transaction) => {
        const month = new Date(transaction.createdAt).getMonth(); // getMonth() returns 0-11
        transactionAmountsByMonth[month] += transaction.amount;
      });

      console.log("transactionAmountsByMonth ", transactionAmountsByMonth);
